...
app.delete("/api/pelis/:id", async (req, res) => {
  ...
  const [rows] = await pool.query(
    `delete from peliculas where id = ${req.params.id}`
  );
  const [rows2] = await pool.query(
    `delete from peli_genero  where peliculaid = ${req.params.id}`
  );
  console.log(delete1);
  res.json(rows);
});
...
app.get("/api/pelis2", async (req, res) => {
  const query =
    "SELECT p.id, p.titulo, p.tmdb_id, p.poster, g.id AS genero_id, g.genero AS genero FROM peliculas p     LEFT JOIN peli_genero pg ON p.id = pg.peliculaid     LEFT JOIN genero g ON pg.generoid = g.id";
